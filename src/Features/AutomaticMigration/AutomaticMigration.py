...
from Core.CustomLogger import (
    set_log_level,
    CustomInMemoryLogHandler,
...
def _get_env_positive_int(var_name, default):
    ...
    try:
        ...
        LOGGER.warning(
            f"Invalid value for env var '{var_name}': '{raw_value}'. "
    ...
    if parsed < 1:
        LOGGER.warning(
            f"Invalid value for env var '{var_name}': '{raw_value}'. "
            f"Expected a positive integer. Using default={default}."
        )
        return default
    return parsed
...
####################################
def mode_AUTOMATIC_MIGRATION(
    source=None,
    target=None,
    show_dashboard=None,
    show_gpth_info=None,
    show_gpth_errors=None,
    parallel=None,
    log_level=None,
):
    with set_log_level(LOGGER, log_level):
        ...
        if parallel is None:
            parallel = ARGS["parallel-migration"]
        ...
        if show_gpth_info is None:
            show_gpth_info = ARGS["show-gpth-info"]
        if show_gpth_errors is None:
            show_gpth_errors = ARGS["show-gpth-errors"]

        # Define the INTERMEDIATE_FOLDER
        INTERMEDIATE_FOLDER = resolve_external_path(
            f"./Automatic_Migration_Push_Failed_{TIMESTAMP}"
        )
        ...
        def get_client_object(client_type):
            ...
            if (
                ...
                and not ARGS["account-id"] > 1
            ):
                return ClassSynologyPhotos(account_id=1)
            elif client_type.lower() in [
                "synology-photos-2",
                "synology-photos2",
                "synology-2",
                "synology2",
                ...
                client_type.lower() in ["synology-photos", "synology"]
                and ARGS["account-id"] > 1
            ):
            ...
            # Return ClassImmichPhotos
                ...
                client_type.lower() in ["immich-photos", "immich"]
                and ARGS["account-id"] > 1
            ):
            ...
            # Return ClassTakeoutFolder
                ...
                contains_zip_files(client_type, log_level=logging.WARNING)
                or contains_takeout_structure(client_type, log_level=logging.INFO)
            ):
                return ClassTakeoutFolder(
                    client_type
                )  # In this clase, client_type is the path to the Takeout Folder

            # Return ClassLocalFolder
            elif Path(client_type).is_dir():
                return ClassLocalFolder(
                    base_folder=client_type
                )  # In this clase, client_type is the path to the base Local Folder
            else:
                raise ValueError(
                    f"{MSG_TAGS['ERROR']}Tipo de cliente no válido: {client_type}"
                )
        ...
        unsupported_text = ""
        if isinstance(source_client, ClassTakeoutFolder) or isinstance(
            source_client, ClassLocalFolder
        ):
        ...
        if not isinstance(source_client, ClassTakeoutFolder):
            LOGGER.warning(
                HELP_TEXTS["AUTOMATIC-MIGRATION"]
            ...
            LOGGER.warning(
                HELP_TEXTS["AUTOMATIC-MIGRATION"]
                .replace("<SOURCE> Cloud Service", f"folder '{source}'")
                .replace("<TARGET>", f"'{target}'")
                .replace("Pulling", "Analyzing and Fixing")
            )
        LOGGER.warning("\n" + "-" * (terminal_width - 11))
        ...
        with set_log_level(
            ...
            LOGGER.info(
                f"🚀 AUTOMATIC MIGRATION JOB STARTED - {source_client_name} ➜ {target_client_name}"
            )
            LOGGER.info(
                f"================================================================================================================================================"
            )
            ...
            if isinstance(source_client, ClassTakeoutFolder):
                if source_client.needs_unzip or source_client.needs_process:
                    LOGGER.info(
                        f"🔢 Source Folder contains a Google Takeout Structure and needs to be processed first. Processing it..."
                    )

                    # Process the Takeout if needed
                    source_client.process(
                        capture_output=show_gpth_info,
                        capture_errors=show_gpth_errors,
                    ...
                    output_metadata_json = os.path.join(
                        FOLDERNAME_EXTRACTED_DATES,
                        f"{TIMESTAMP}_takeout_output_dates_metadata_final.json",
                    )
                    if os.path.isfile(output_metadata_json):
                        source_client._ensure_analyzer(
                            metadata_json_file=output_metadata_json, log_level=log_level
                        )
                    else:
            ...
            if isinstance(target_client, ClassTakeoutFolder):
                if target_client.needs_unzip or target_client.needs_process:
                    LOGGER.info(
                        f"🔢 Target Folder contains a Google Takeout Structure and needs to be processed first. Processing it..."
                    )
                    target_client.process(
                        capture_output=show_gpth_info,
                        capture_errors=show_gpth_errors,
                        print_messages=print_messages,
                    )
            ...
            # ---------------------------------------------------------------------------------------------------------
            try:
                parallel_automatic_migration(
                    source_client=source_client,
                    target_client=target_client,
                    temp_folder=INTERMEDIATE_FOLDER,
...
def parallel_automatic_migration(
    source_client,
    target_client,
    temp_folder,
    SHARED_DATA,
            ...
            if parallel:
                ...
                while push_queue.qsize() >= 100:
                    while push_queue.qsize() > 25:
                        time.sleep(
                            1
                        )  # Hacemos pausas de 1s hasta que la cola se vacíe (25 elementos)
                        # SHARED_DATA.info['assets_in_queue'] = push_queue.qsize()
    ...
    def is_asset_in_queue(queue, path):
        ...
        with queue.mutex:
            return any(
                item["asset_file_path"].lower() == path.lower()
    ...
    def main_thread(parallel=None, log_level=logging.INFO):
        ...
        with set_log_level(
            ...
            with_filters = False
            if (
                ARGS.get("filter-by-type", None)
                or ARGS.get("filter-from-date", None)
                or ARGS.get("filter-to-date", None)
                or ARGS.get("filter-by-country", None)
                or ARGS.get("filter-by-city", None)
                or ARGS.get("filter-by-person", None)
            ):
                with_filters = True
            ...
            LOGGER.info(
                f"🚀 Starting Automatic Migration Process: {source_client_name} ➜ {target_client_name}..."
            )
            LOGGER.info(f"Source Client  : {source_client_name}")
            LOGGER.info(f"Target Client  : {target_client_name}")
            ...
            LOGGER.info(
                f"Analyzing Source client and Applying filters. This process may take some time, please be patient..."
            )
            ...
            try:
                LOGGER.info(
                    f"Retrieving Albums on '{source_client_name}' matching filters criteria (if any). This process may take some time, please be patient..."
                )
                all_albums = source_client.get_albums_including_shared_with_user(
                    filter_assets=with_filters, log_level=logging.WARNING
                )
            except Exception as e:
                LOGGER.error(
                    f"Error Retrieving All Albums from '{source_client_name}'. - {e}"
                )
            LOGGER.info(
                f"{len(all_albums)} Albums found on '{source_client_name}' matching filters criteria"
            )
            for album in all_albums:
                album_id = album["id"]
                album_name = album["albumName"]
                album_passphrase = album.get(
                    "passphrase"
                )  # Obtiene el valor si existe, si no, devuelve None
                permissions = (
                    album.get("additional", {})
                    .get("sharing_info", {})
                    .get("permission", [])
                )  # Obtiene el valor si existe, si no, devuelve None
                if permissions:
                    album_shared_role = permissions[0].get(
                        "role"
                    )  # Obtiene el valor si existe, si no, devuelve None
                else:
                ...
                if album_shared_role.lower() == "view":
                    LOGGER.info(
                        f"Album '{album_name}' cannot be pulled because is a blocked shared album. Skipped!"
                    )
                    total_albums_blocked_count += 1
                    total_assets_blocked_count += album.get("item_count")
                    try:
                        blocked_assets.extend(
                            source_client.get_all_assets_from_album_shared(
                                album_id=album_id,
                        ...
                        LOGGER.error(
                            f"Error Retrieving Shared Albums's Assets from '{source_client_name}' - {e}"
                        )
            # Get all assets and filter out those blocked assets (from blocked shared albums) if any
            try:
                all_no_albums_assets = source_client.get_all_assets_without_albums(
                    log_level=logging.WARNING
                )
            except Exception as e:
                LOGGER.error(
                    f"Error Retrieving Assets without albums from '{source_client_name}' - {e}"
                )
            try:
                all_albums_assets = source_client.get_all_assets_from_all_albums(
                    log_level=logging.WARNING
                )
            except Exception as e:
                LOGGER.error(
                    f"Error Retrieving Albums's Assets from '{source_client_name}' - {e}"
                )

            all_supported_assets = all_no_albums_assets + all_albums_assets
            blocked_assets_ids = {asset["id"] for asset in blocked_assets}
            filtered_all_supported_assets = [
                asset
                for asset in all_supported_assets
                if asset["id"] not in blocked_assets_ids
            ]

            all_photos = [
                asset
                for asset in filtered_all_supported_assets
                if asset["type"].lower() in image_labels
            ]
            all_videos = [
                asset
                for asset in filtered_all_supported_assets
                if asset["type"].lower() in video_labels
            ]
            all_assets = all_photos + all_videos
            all_metadata = [
                asset
                for asset in filtered_all_supported_assets
                if asset["type"].lower() in metadata_labels
            ]
            all_sidecar = [
                asset
                for asset in filtered_all_supported_assets
                if asset["type"].lower() in sidecar_labels
            ]
            all_invalid = [
                asset
                for asset in filtered_all_supported_assets
                if asset["type"].lower() in ["unknown"]
            ]
            ...
            cpu_total_threads = os.cpu_count() or 1
            LOGGER.info(f"")
            ...
            max_push_workers = _get_env_positive_int(
                "PHOTOMIGRATOR_MAX_PUSH_WORKERS", calculated_push_workers
            )
            num_push_threads = max(1, min(calculated_push_workers, max_push_workers))
            LOGGER.info(
                f"Launching {num_push_threads} Push workers in parallel "
                f"(calculated={calculated_push_workers}, cap={max_push_workers}; "
                f"override with env PHOTOMIGRATOR_MAX_PUSH_WORKERS)."
            )

            pull_threads = [
                threading.Thread(
                    target=puller_worker, kwargs={"parallel": parallel}, daemon=True
                )
            ...
            migration_formatted_duration = str(
                timedelta(
                    seconds=round((end_time - migration_start_time).total_seconds())
                )
            )
            total_formatted_duration = str(
                timedelta(
                    seconds=round(
                        (end_time - SHARED_DATA.info["start_time"]).total_seconds()
                    )
                )
            )
            ...
            LOGGER.info(
                f"Pulled Albums               : {SHARED_DATA.counters['total_pulled_albums']}"
            )
            LOGGER.info(
                f"Pushed Albums               : {SHARED_DATA.counters['total_pushed_albums']}"
            )
            LOGGER.info(
                f"Pulled Assets               : {SHARED_DATA.counters['total_pulled_assets']} (Photos: {SHARED_DATA.counters['total_pulled_photos']}, Videos: {SHARED_DATA.counters['total_pulled_videos']})"
            )
            LOGGER.info(
                f"Pushed Assets               : {SHARED_DATA.counters['total_pushed_assets']} (Photos: {SHARED_DATA.counters['total_pushed_photos']}, Videos: {SHARED_DATA.counters['total_pushed_videos']})"
            )
            LOGGER.info(
                f"Push Duplicates (skipped)   : {SHARED_DATA.counters['total_push_duplicates_assets']}"
            )
            LOGGER.info(
                f"Pull Failed Assets          : {SHARED_DATA.counters['total_pull_failed_assets']}"
            )
            LOGGER.info(
                f"Push Failed Assets          : {SHARED_DATA.counters['total_push_failed_assets']}"
            )
            LOGGER.info(f"")
        ...
        with set_log_level(LOGGER, log_level):
            ...
            try:
                albums = source_client.get_albums_including_shared_with_user(
                    filter_assets=with_filters, log_level=logging.ERROR
                )
            except Exception as e:
                LOGGER.error(
                    f"Error Retrieving All Albums - {e} \n{traceback.format_exc()}"
                )
            ...
            for album in albums:
                ...
                if permissions:
                    ...
                    album_shared_role = ""
                is_shared = (
                    album_passphrase is not None and album_passphrase != ""
                )  # Si tiene passphrase, es compartido
                ...
                try:
                    ...
                    LOGGER.error(
                        f"Error Retrieving All Assets from album {album_name} - {e} \n{traceback.format_exc()}"
                    )
                ...
                try:
                    for asset in album_assets:
                        ...
                        if pulled_assets > 0:
                            LOGGER.info(
                                f"Asset Pulled    : '{os.path.basename(local_file_path)}'"
                            )
                            ...
                            unique = enqueue_unique(
                                push_queue, asset_dict, parallel=parallel
                            )  # Añadimos el asset a la cola solo si no se había añadido ya un asset con el mismo 'asset_file_path'
                            if not unique:
                                LOGGER.info(
                                    f"Asset Duplicated: '{os.path.basename(local_file_path)}' from Album '{album_name}. Skipped"
                                )
                                SHARED_DATA.counters[
                                    "total_push_duplicates_assets"
                                ] += 1
                                # Solo borramos si ya no está en la cola (ignorando mayúsculas)
                                if not is_asset_in_queue(
                                    push_queue, local_file_path
                                ) and os.path.exists(local_file_path):
                                    try:
                                        os.remove(local_file_path)
                                    except Exception as e:
                                        LOGGER.warning(
                                            f"Could not remove file '{local_file_path}': {e}"
                                        )
                        else:
                            LOGGER.warning(
                                f"Asset Pull Fail : '{os.path.basename(local_file_path)}' from Album '{album_name}'"
                            )
            ...
            try:
                ...
                LOGGER.error(
                    f"Error Retrieving All Assets without Albums - {e} \n{traceback.format_exc()}"
                )
            ...
            try:
                ...
                for asset in assets_no_album:
                    ...
                    try:
                        ...
                        LOGGER.error(
                            f"Asset Pull Error: '{os.path.basename(local_file_path)}' - {e} \n{traceback.format_exc()}"
                        )
                        SHARED_DATA.counters["total_pull_failed_assets"] += 1
                        if asset_type.lower() in video_labels:
                            SHARED_DATA.counters["total_pull_failed_videos"] += 1
                    ...
                    if pulled_assets > 0:
                        ...
                        if not unique:
                            LOGGER.info(
                                f"Asset Duplicated: '{os.path.basename(local_file_path)}'. Skipped"
                            )
                        ...
                        LOGGER.warning(
                            f"Asset Pull Fail : '{os.path.basename(local_file_path)}'"
                        )
                        SHARED_DATA.counters["total_pull_failed_assets"] += 1
                        if asset_type.lower() in video_labels:
                            SHARED_DATA.counters["total_pull_failed_videos"] += 1
                ...
                try:
                    ...
                    try:
                        ...
                        asset_id, isDuplicated = target_client.push_asset(
                            file_path=asset_file_path, log_level=logging.ERROR
                        )
                        ...
                        if asset_id:
                            ...
                            if album_name:
                                LOGGER.error(
                                    f"Asset Push Fail : '{os.path.basename(asset_file_path)}'. Album: '{album_name}'"
                                )
                            else:
                                LOGGER.error(
                                    f"Asset Push Fail : '{os.path.basename(asset_file_path)}'"
                                )
                        ...
                        if move_assets:
                            source_client.remove_assets(
                                asset_ids=asset["asset_id"], log_level=log_level
                            )
                        ...
                        if asset_pushed and os.path.exists(asset_file_path):
                            try:
                                ...
                                LOGGER.warning(
                                    f"Could not remove local file '{asset_file_path}' after push: {e}"
                                )
                    except Exception as e:
                        ...
                        with album_creation_lock:
                            if album_name not in created_albums:
                                exists, aid = target_client.album_exists(
                                    album_name=album_name, log_level=logging.ERROR
                                )
                                if not exists:
                                    ...
                                    LOGGER.info(
                                        f"Album Created   : '{album_name}' by pusher_worker={worker_id}"
                                    )
                            ...
                            try:
                                ...
                                if album_name not in processed_albums:
                                    processed_albums.append(
                                        album_name
                                    )  # Lo incluimos en la lista de albumes procesados
                                    SHARED_DATA.counters["total_pushed_albums"] += 1
                                    SHARED_DATA.counters["total_pushed_albums"] = min(
                                        SHARED_DATA.counters["total_pushed_albums"],
                                        SHARED_DATA.counters["total_pulled_albums"],
                                    )  # Avoid to set total_pushed_albums > total_pulled_albums
...
###########################
def start_dashboard(
    migration_finished, SHARED_DATA, parallel=True, step_name="", log_level=None
):
    with set_log_level(LOGGER, log_level):
        import time
        ...
        def exception_hook(exctype, value, tb):
            ...
            LOGGER.warning(
                f"{step_name}Unhandled exception in Live Dashboard:\n{error_msg}"
            )
            try:
                console.show_cursor()
            except:
        ...
        try:
            ...
            if terminal_height < MIN_TERMINAL_HEIGHT:
                LOGGER.info(
                    f"Cannot display Live Dashboard because the detected terminal height = {terminal_height} and the minumum needed height = {MIN_TERMINAL_HEIGHT}. Continuing without Live Dashboard..."
                )
                ARGS["dashboard"] = (
                    False  # Set this argument to False to avoid use TQDM outputs as if a Interactive Terminal (isatty() = True)
                )
                return

            if terminal_width < MIN_TERMINAL_WIDTH:
                LOGGER.info(
                    f"Cannot display Live Dashboard because the detected terminal width = {terminal_width} and the minumum needed width = {MIN_TERMINAL_WIDTH}. Continuing without Live Dashboard..."
                )
                ARGS["dashboard"] = (
                    False  # Set this argument to False to avoid use TQDM outputs as if a Interactive Terminal (isatty() = True)
                )
            ...
            for handler in list(
                LOGGER.handlers
            ):  # Hacer una copia de la lista para evitar problemas al modificarla
            ...
            memory_handler.setFormatter(
                CustomConsoleFormatter(fmt="%(message)s", datefmt="%Y-%m-%d %H:%M:%S")
            )
            ...
            layout["header_panel"].update(
                Panel(f"[gold1]{header}[/gold1]", border_style="gold1", expand=True)
            )
            ...
            layout["title_panel"].update(
                Panel(f"🚀 {title}", border_style="bright_blue", expand=True)
            )
            ...
            def build_info_panel(clean_queue_history=False):
                ...
                filled_blocks = min(
                    int((current_queue_size / 100) * BAR_WIDTH), BAR_WIDTH
                )
                ...
                info_data = [
                    ...
                    (
                        "🔒 Blocked Albums",
                        SHARED_DATA.info.get("total_albums_blocked", 0),
                    ),
                ...
                for label, value in info_data:
                    table.add_row(
                        f"[bright_magenta]{label:<17}: [/bright_magenta]",
            ...
            for label, (bar, completed_label, total_label) in pull_bars.items():
                ...
                pull_tasks[label] = bar.add_task(
                    label,
                    completed=SHARED_DATA.counters.get(completed_label),
                    total=SHARED_DATA.info.get(total_label, 0),
                )

            # PUSHS (Green)
            push_bars = {  # Dicccionario de Tuplas (bar, etiqueta_contador_completados, etiqueta_contador_totales)
                "🎯 Pushed Assets": (
                    create_progress_bar("green"),
                    "total_pushed_assets",
                    "total_assets",
                ),
                "📷 Pushed Photos": (
            ...
            def build_pull_panel():
                ...
                for label, (bar, completed_labeld, total_label) in pull_bars.items():
                    ...
                    bar.update(
                        pull_tasks[label],
                        completed=SHARED_DATA.counters.get(completed_labeld),
                        total=SHARED_DATA.info.get(total_label, 0),
                    )
            ...
            def build_push_panel():
                ...
                for label, counter_label in failed_pushs.items():
                    value = SHARED_DATA.counters[counter_label]
                    table.add_row(
                        f"[green]{label:<16}:[/green]", f"[green]{value}[/green]"
                    )
                return Panel(
                    table,
            ...
            logs_panel_height = (
                terminal_height - fixed_heights - 2
            )  # Espacio restante. Restamos 2 para quitar las líneas del borde superior e inferior del panel de Logs
            ...
            def build_log_panel():
                ...
                try:
                    while True:
                        ...
                        try:
                            line = (
                                SHARED_DATA.logs_queue.get_nowait()
                        ...
                        if "warning :" in line_lower:
                            ...
                            any(
                                word in line_lower
                                for word in ("push", "created", "duplicated")
                            )
                            and not "pull" in line_lower
                        ):
            ...
            with Live(
                layout, refresh_per_second=1, console=console, vertical_overflow="crop"
            ):
                try:
                    update_title_panel()
                    ...
                    while not migration_finished.is_set():
                        SHARED_DATA.info["elapsed_time"] = str(
                            timedelta(
                                seconds=round(
                    ...
                    layout["info_panel"].update(
                        build_info_panel(clean_queue_history=True)
                    )  # Limpiamos el histórico de la cola
                    layout["pulls_panel"].update(build_pull_panel())
                    layout["pushs_panel"].update(build_push_panel())
                    layout["logs_panel"].update(build_log_panel())
                    ...
                    LOGGER.warning(
                        f"{step_name}Exception during Live Dashboard execution:\n{err}"
                    )
                    try:
                        console.show_cursor()
                    except:
